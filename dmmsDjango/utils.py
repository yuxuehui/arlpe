...
class ReplayBuffer(object):
    def __init__(self, state_dim, action_dim, personal_dim, device, max_size=int(1e6)):
        ...
        self.personal_feature = np.zeros((max_size, personal_dim))

        self.device = device

    def add(self, state, action, next_state, reward, done, personal_feature):
        self.state[self.ptr] = state
        self.action[self.ptr] = action
        self.next_state[self.ptr] = next_state
        self.reward[self.ptr] = reward
        self.not_done[self.ptr] = 1. - done
        self.personal_feature[self.ptr] = personal_feature

        self.ptr = (self.ptr + 1) % self.max_size  # 当经验回访池存满，从头开始覆盖存储
    ...
    def sample(self, batch_size):
        ...
        return (
            ...
            torch.FloatTensor(self.personal_feature[ind]).to(self.device)
        )

    def len(self):#2021-3-1加
        return self.size
    ...
    def save(self, save_folder):
        ...
        np.save(f"{save_folder}_personal_feature.npy", self.personal_feature[:self.size])
    ...
    def load(self, save_folder, size=-1):
        ...
        self.personal_feature[:self.size] = np.load(f"{save_folder}_personal_feature.npy")[:self.size]