...
def log(s, with_prefix=True, with_timestamp=True):
    ...
    if with_timestamp:
        ...
        # timestamp = now.strftime('%Y-%m-%d %H:%S.%f %Z')
        timestamp = now.strftime('%Y-%m-%d %H:%S.%f')
...
def save_itr_params(itr, params_dict):
    ...
    if _snapshot_dir:
        # print("_snapshot_dir: ", _snapshot_dir)
        # print("_snapshot_mode: ", _snapshot_mode)
        # print("names: ", names)
        ...
        if _snapshot_mode == 'all':
            ...
            # override previous params 覆盖之前的参数
