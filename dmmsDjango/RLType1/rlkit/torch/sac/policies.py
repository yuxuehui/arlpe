...
class TanhGaussianPolicy(Mlp, ExplorationPolicy):
    ...
    def __init__(
        ...
        if std is None:
            ...
            assert LOG_SIG_MIN <= self.log_std <= LOG_SIG_MAX  # 如果 不等式不成立，则raise一个AssertionError出来

    def get_action(self, obs, deterministic=False):
        # in_ = torch.cat([obs, z, p], dim=1)
    ...
    def forward(
        ...
        pre_tanh_value = None
        if deterministic:   # 采用确定值
            action = torch.tanh(mean)   # 直接过激活层
        else:
        ...
        with open('/home/Data/yanlian/yxhfile/dmmsDjSA_end/dmmsDjango/results/dataDeal2.txt', 'a+') as dfile:
            dfile.write('\tmean:' + str(mean) + '\tstd:' + str(std) + '\tobs:' + str(obs) + '\t')
            # dfile.write('\tmean:' + str(mean) + '\tstd:' + str(std) + '\t')
